...
class MusicDance2Keyframe:
    ...
    def save_camera_keyframe(self, c_kf, epoch, render_out, wav_path):
        ...
        norm_wav_path = os.path.normpath(wav_path) 
        pathparts = norm_wav_path.split(os.sep)
        camera_name = 'ck' + pathparts[-1][1:-6] + '.json'
        
        
        keyframe_mask = c_kf[0, :, 0].clone()
        keyframe_pos = torch.nonzero(c_kf == 1)[:, 1]
        keyframe_pos = keyframe_pos.tolist()
        filtered_pos = []
        min_len = 60 # Define minimum keyframe interval

        for i in range(len(keyframe_pos)):
            if i == 0 or (keyframe_pos[i] - keyframe_pos[i - 1] > min_len):
                filtered_pos.append(keyframe_pos[i])

        new_mask = [0] * keyframe_mask.shape[0]
        for idx in filtered_pos:
            if idx < len(new_mask):
                new_mask[idx] = 1
                
        save_path = os.path.join(render_out, str(epoch), camera_name)
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        

        with open(save_path, 'w') as ckf:
            json.dump({
                "KeyframeMask": new_mask,
                "KeyframePos": filtered_pos
            }, ckf)
